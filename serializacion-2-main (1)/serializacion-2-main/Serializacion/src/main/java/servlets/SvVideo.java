...
private void guardarVideosEnArchivo() {
    try (BufferedWriter writer = new BufferedWriter(new FileWriter("videos.csv"))) {
        for (Video video : misVideos) {
            String linea = String.format("%d,%s,%s,%s,%s,%s,%s",
                    video.getIdVideo(), video.getTitulo(), video.getAutor(),
                    video.getAnio(), video.getCategoria(), video.getLetra(), video.getUrl());
            writer.write(linea);
            writer.newLine();
        }
    } catch (IOException e) {
